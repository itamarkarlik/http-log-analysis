...
```spl
index="soc-splunk-lab" sourcetype="http_logs" src_ip="192.168.202.110" status_code=200
| search uri="*../*"
| stats count by uri
| sort - count
```
...
Identify traversal-style requests associated with successful HTTP 200 responses generated by the suspicious source IP.

### Result

The analysis identified multiple traversal-style requests that received successful HTTP 200 responses.

Observed requests targeted potentially sensitive system resources including:

- `/etc/passwd`
- `winnt/system32/ipconfig.exe`

The activity also targeted multiple web application paths including phpMyAdmin-related resources and CGI-style application paths.

The repeated traversal sequences and successful responses suggest automated vulnerability probing and potential exposure of vulnerable web resources.
